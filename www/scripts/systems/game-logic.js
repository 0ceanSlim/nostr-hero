...
async function moveToLocation(locationId) {
    ...
    try {
        ...
        await window.gameAPI.move(cityId, districtKey, '');
...
async function useItem(itemId, slot) {
    console.log('Using item:', itemId, 'from slot:', slot);
    ...
    try {
        ...
        await window.gameAPI.useItem(itemId, slot);
