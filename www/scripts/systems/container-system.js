// Container System - handles opening containers and managing their contents

let currentOpenContainer = null;

// Open a container and show its contents
async function openContainer(itemId, fromSlot) {
    console.log('Opening container:', itemId, 'from slot:', fromSlot);
    ...
    console.log('Container properties:', { containerSlots, allowedTypes, allowedItems });
    ...
    const gameState = await getGameState(true);
    ...
    if (inventory && inventory.general_slots) {
        const slot = inventory.general_slots[fromSlot];
        if (slot && slot.item === itemId && slot.contents) {
    ...
    renderContainerSlots(slotsGrid, containerSlots, containerData);
...
async function renderContainerSlots(grid, totalSlots, contents) {
    ...
    for (let i = 0; i < totalSlots; i++) {
        ...
        if (slotItem && slotItem.item) {
            // Get item data
            const itemData = await getItemByIdAsync(slotItem.item);
            if (itemData) {
                // Set draggable
...
// Show UI to add item to container
function showAddItemToContainer(slotIndex) {
    // TODO: Implement item selection UI
    showMessage('💡 Drag items from inventory to add to container (coming soon)', 'info');
}
...
function handleContainerDragStart(e, itemId, slotIndex) {
    ...
    console.log(`📦 Drag start from container slot ${slotIndex}: ${itemId}`);
}
...
async function handleContainerDrop(e, toSlotIndex) {
    ...
    if (containerDraggedItem !== null && containerDraggedFromSlot !== null) {
        ...
        // Move item within container (not implemented in backend yet - TODO)
        console.log('⚠️ Moving items within containers not yet implemented');
        return;
    }

    // Check if dragging from inventory to container
    if (window.inventoryDragState && window.inventoryDragState.itemId) {
        const itemId = window.inventoryDragState.itemId;
        const fromSlot = window.inventoryDragState.fromSlot;
        const fromType = window.inventoryDragState.fromType;

        console.log(`📦 Adding to container: ${itemId} from ${fromType}[${fromSlot}]`);

        // Call backend API to add item to container
        try {
            const result = await window.gameAPI.sendAction('add_to_container', {
                item_id: itemId,
                from_slot: fromSlot,
            ...
            if (result.success) {
                // Show success message with color from API
                if (window.showActionText) {
                    window.showActionText(result.message, result.color || 'green');
                }
                ...
                // Show error message
                if (window.showActionText) {
                    window.showActionText(result.error, result.color || 'red');
                }
            }
        } catch (error) {
            console.error('❌ Failed to add item to container:', error);
            if (window.showActionText) {
                window.showActionText('Failed to add item to container', 'red');
            }
        }
    }
}

// Note: Validation logic moved to Go backend (inventory.go)
// JavaScript only handles UI and API calls
...
async function removeFromContainer(slotIndex) {
    ...
    try {
        ...
        const result = await window.gameAPI.sendAction('remove_from_container', {
            item_id: item.item,
            container_slot: currentOpenContainer.fromSlot,
        ...
        if (window.showActionText) {
            window.showActionText('Failed to remove item from container', 'red');
        }
    }
}
...
// Note: saveContainerChanges() removed - state is now managed by Go backend
...
async function closeContainer() {
    ...
    // Give DOM time to fully render before rebinding
    await new Promise(resolve => setTimeout(resolve, 50));

    // CRITICAL: Rebind all inventory events after modal closes
    if (window.inventoryInteractions && window.inventoryInteractions.bindInventoryEvents) {
        console.log('🔄 Rebinding inventory events after container close...');
