...
### Class Resources Definition (`game-data/systems/class-resources.json`)

Defines resource type, display, and mechanics for each class:
...
### Character Save Data (Minimal)

**For Casters** (mana persists):
```json
{
  "class": "wizard",
  "level": 12,
...
**For Martials** (resource derived from game-data, resets each combat):
```json
{
  "class": "fighter",
  "level": 12,
...
Resource type, max, and regen are looked up from `class-resources.json` based on class.
Abilities unlocked are derived from class + level (no need to store).
Ability uses are tracked only during active combat (not persisted to save file).
...
## Finalized Decisions (2026-01-26)

### Half-Casters (Paladin, Ranger)

**Decision**: Half-casters use the standard mana system. They may get supplementary abilities or effects later that complement their spellcasting, but they are not part of the martial resource system.

### Resource Persistence

**Decision**: Martial resources are combat-focused and reset/initialize at combat start:
- **Fighter/Monk/Rogue**: Start combat at max resource
- **Barbarian**: Starts combat at 0 Rage (builds through taking damage)

Outside combat, martial resource bars display at their `combat_start` value (full for most, empty for Barbarian).

### UI Bar Colors (Confirmed)

| Class | Resource | Color Hex | Visual |
|-------|----------|-----------|--------|
| Casters | Mana | `#2563eb` | Blue (existing) |
| Fighter | Stamina | `#22c55e` | Green |
| Barbarian | Rage | `#f97316` | Orange |
| Monk | Ki | `#eab308` | Gold |
| Rogue | Cunning | `#a855f7` | Purple |

### Save File Architecture

**Decision**: Minimal save file approach - derive resource configuration from game-data.

- Casters: Use existing `mana`/`max_mana` fields (persists between combats)
- Martials: `mana: 0, max_mana: 0` - resource type/max/regen derived from `class-resources.json`
- No new save file fields needed for martial resources

### Spells Tab Usage

**Decision**: The existing spells tab (currently non-functional) will be used for:
- **Casters**: Spells and spell slot management
- **Martials**: Ability information and usage (during combat)
- **Half-casters**: Hybrid view with both spells and any supplementary abilities
