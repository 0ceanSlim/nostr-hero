 ...
 *
 * Deterministic character generation using Nostr public key as seed.
 * Generates D&D 5e-style characters with stats, equipment, and spells.
 *
 * @module logic/characterGenerator
 */
...
/**
 * Nostr Character Generator Class
 */
class NostrCharacterGenerator {
    ...
    async initialize() {
        try {
            logger.info('Loading character generation data...');
            ...
            const [
                ...
                fetch(`${API_BASE_URL}/weights`),
                fetch(`${API_BASE_URL}/introductions`),
                fetch(`${API_BASE_URL}/starting-gear`),
            ...
            logger.info('Character generation data loaded');
            return true;
        } catch (error) {
            logger.error('Failed to load character generation data:', error);
            throw error;
        }
    }

    /**
     * Generate a complete character from a Nostr public key
     * @param {string} npub - Nostr public key (npub format)
    ...
    async generateCharacter(npub) {
        ...
        logger.info('Generating character for:', npub);

        // Use the existing /api/character endpoint that has the correct algorithm
        const response = await fetch(`${API_BASE_URL}/character?npub=${encodeURIComponent(npub)}`);
        if (!response.ok) {
        ...
        logger.info('Generated character from API:', character);
        return character;
    }
     ...
     * @returns {Object} Introduction object with text sections
     */
     ...
     * @returns {string} Equipment introduction text
     */
     ...
     * @param {Object} character - Character object
     * @returns {Object} Equipment result with inventory, equipment, choices, and pack_choice
     */
    generateStartingEquipment(character) {
        ...
        if (!classGearData) {
            logger.warn('No starting gear found for class:', characterClass);
        ...
        if (gearConfig.equipment_choices) {
            logger.debug(`Processing ${gearConfig.equipment_choices.length} equipment choices for ${characterClass}`);
            gearConfig.equipment_choices.forEach((equipChoice, index) => {
                logger.debug(`Choice ${index}: ${equipChoice.options.length} options`);
                const choice = {
                    id: `choice-${index}`,
                    ...
                    options: equipChoice.options.map((opt, optIndex) => {
                        ...
                        logger.warn('Unknown option type:', opt);
        ...
        logger.debug(`Generated ${choices.length} equipment choices for ${characterClass}:`, choices);
        return {
            inventory: inventory,
    ...
    generateStartingGold(background) {
        if (!this.startingGold || !this.startingGold['starting-gold']) {
            logger.warn('Starting gold data not loaded');
            return 0;
        }
        ...
        if (entry) {
            ...
            logger.warn('No starting gold found for background:', background);
     ...
     * @param {Object} character - Character object
     * @returns {string} Starting city ID
     */
    generateStartingCity(character) {
        if (!this.racialStartingCities) {
            logger.warn('Racial starting cities data not loaded');
        ...
        if (startingCity) {
            ...
            logger.warn('No starting city found for race:', character.race);
            return 'kingdom'; // Default to kingdom
        }
    }
    ...
    generateStartingSpells(character) {
        if (!this.startingSpells) {
            logger.warn('Spell data not loaded');
            return [];
        }

        const classSpells = this.startingSpells[character.class.toLowerCase()];
        if (!classSpells) {
            logger.debug(`${character.class} has no starting spells`);
            return [];
        }
        ...
        logger.debug(`Generated ${knownSpells.length} known spells for ${character.class}:`, knownSpells);
        return knownSpells;
    }
     ...
     * @param {Object} character - Character object
     * @returns {Object} Spell slots object by level
     */
    generateSpellSlots(character) {
        if (!this.spellSlots) {
            logger.warn('Spell slots data not loaded');
            return {};
        }

        const classSlots = this.spellSlots[character.class.toLowerCase()];
        if (!classSlots) {
            logger.debug(`${character.class} is not a spellcasting class or has no spell slots`);
            return {};
        }

        // Get slot counts for level 1
        const level1SlotCounts = classSlots["1"] || {};
        logger.debug(`Spell slot counts for ${character.class} level 1:`, level1SlotCounts);
        ...
        logger.debug(`Generated spell slots for ${character.class}:`, spellSlots);
        return spellSlots;
    }
...
export const characterGenerator = new NostrCharacterGenerator();
export { NostrCharacterGenerator };

logger.debug('Character generator module loaded');
