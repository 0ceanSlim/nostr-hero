...
func GenerateCharacter(hexKey string) (types.Character, error) {
	...
	if backgrounds, ok := weightData.BackgroundWeightsByClass[class]; ok {
		for background, weight := range backgrounds {
			backgroundOptions = append(backgroundOptions, background)
			backgroundWeights = append(backgroundWeights, weight)
		}
	} else {
		return types.Character{}, fmt.Errorf("no valid background found for class: %s", class)
	}
