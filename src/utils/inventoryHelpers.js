/**
 * Inventory Helper Utilities Module
 *
 * Shared functions for creating and managing character inventories.
 * Handles item stacking, pack unpacking, and inventory creation logic.
 *
 * @module utils/inventoryHelpers
 */

import { logger } from '../lib/logger.js';
import { getItemByIdAsync } from './itemHelpers.js';
...
export async function unpackItem(packId) {
  try {
    ...
    const packData = await getItemByIdAsync(packId);
    if (packData) {
...
export async function createInventoryFromItems(allItems) {
  ...
  for (let i = remainingItems.length - 1; i >= 0; i--) {
    const item = remainingItems[i];
    const itemData = await getItemByIdAsync(item.item);
...
if (typeof window !== 'undefined') {
  ...
  window.unpackItem = unpackItem;
  window.addToGeneralSlotOrBag = addToGeneralSlotOrBag;
  window.createInventoryFromItems = createInventoryFromItems;
}

logger.debug('Inventory helpers module loaded');
