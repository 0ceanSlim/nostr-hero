/**
 * Item Helper Utilities Module
 *
 * Shared functions for loading and accessing item data from the database.
 * Provides item lookups, caching, and formatting utilities.
 *
 * @module utils/itemHelpers
 */
...
export async function loadItemsFromDatabase() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      logger.debug(`Loaded ${itemsDatabaseCache.length} items from database`);
      return itemsDatabaseCache;
    }
  } catch (error) {
 ...
 */
export async function getItemByIdAsync(itemId) {
  try {
...
if (typeof window !== 'undefined') {
  ...
  window.getItemByIdAsync = getItemByIdAsync;
  window.clearItemsCache = clearItemsCache;
  window.getItemImageName = getItemImageName;
  window.getItemStats = getItemStats;
}

logger.debug('Item helpers module loaded');
